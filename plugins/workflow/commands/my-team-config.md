...
```
my-team config에 들어갈 항목을 안내합니다. 형식은 자유로 적어주세요.

[팀 단위 — 모두 필수]
- team_name: 팀 이름 (kebab-case 권장, 예: order-cache-poc)
- team_goal: 팀이 무엇을 달성하려고 모였는지 (자유서술 1~3문장)
- worker_count: 워커 명수 (1~10)

[워커마다 — 워커 수만큼 반복]
- name (필수): 워커 식별자 ([a-zA-Z0-9-]+ 패턴)
- cwd (필수): 워커가 작업할 디렉토리 절대경로 또는 ~/로 시작
- agent_type (필수): claude / codex / gemini / cursor 중 하나
- 합류 이유 / 특성 (필수): 이 워커가 팀에 합류한 이유 (자유서술)

[자동으로 채워지는 항목 — 입력 불필요]
- launch_args: agent_type에 따라 권장값을 자동 주입 (위험 플래그 시 별도 확인)
- description: 합류 이유에서 한 줄 요약을 LLM이 초안 생성, 사용자가 확인

자유롭게 적어주세요. 예시:

3명
팀 생성 목적: 주문 캐시 PoC. Redis를 도입해 응답 지연을 줄인다.
첫 번째 워커: alpha @ ~/work/api / claude — 백엔드 API 담당, 캐시 키 설계
두 번째 워커: beta @ ~/work/order / codex — invalidation 로직 책임
세 번째 워커: gamma @ ~/work/web / gemini — 프론트 캐시 TTL UI
```

입력을 받으면 LLM이 파싱한다. **형식이 다르더라도 의미가 명확하면 그대로 진행한다.** 다음 경우에만 사용자에게 되묻는다.

- 필수 항목 중 명백히 누락된 게 있을 때 (예: 워커의 cwd가 어디에도 안 보임) — 누락된 항목만 짚어서 추가 질문.
- 워커 수 선언(N명)과 실제 적힌 워커 수가 다를 때 — 어느 쪽이 맞는지 확인.

LLM이 의심스럽게 파싱한 경우(예: `agent_type`을 추측해 채운 경우)에도 한 번 더 확인한다.

## Step 2 — 검증

파싱한 결과를 워커 단위 요약(예: "alpha: ~/work/api, claude — 백엔드 API 담당")으로 사용자에게 한 번 보여준 뒤 다음을 자동 확인한다. 위반이 있으면 어느 워커의 무엇이 문제인지 짚어서 1회 수정 요청한다.
...
최종 JSON을 코드블록으로 보여주고 "저장할까요? (y/N)"로 1회 확인 받는다. 키 순서는 `team_name`, `workers[].{name, cwd, agent_type, launch_args, description, extra_prompt}` 고정.
