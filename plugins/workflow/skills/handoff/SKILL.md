...
2. **features/ 동기화**: 연결된 `features/<feature-name>/task-index.md`의 *TODO 섹션*에서 이번 세션이 완료한 항목을 체크하고, 새로 발견한 슬라이스 외 작업을 추가한다. 또한 `task-index.md`와 *현재 진행 중인* `tdd-state/slice-N.md`를 Relevant Files에 자동 포함한다 (plan/tdd가 만든 자산).
...
- 적용된 변경: features/<feature-name>/task-index.md (TODO 섹션)
  - 체크: N개 (...)
  - 추가: M개 (...)
- 슬라이스 진행 마커 변경: tdd가 이미 토글한 항목 요약 (없으면 "(없음 — tdd가 마커 토글 미적용)")
...
3. **트랜스크립트에서 추출**
   ...
   - (슬라이스 자체의 진행 마커는 tdd가 이미 토글했어야 함. 누락된 게 있으면 보고만)
4. **Relevant Files 선정**
...
7. **task-index.md TODO 섹션 변경 후보 사용자 확인 + 적용** (Step 2에서 features/ 슬롯을 찾은 경우만)
   ...
   - **사용자 확인 없이 자동 수정 금지** (task-index.md는 source of truth). Slices·Decisions 섹션은 절대 수정 X (plan/tdd 영역).
...
handoff는 `task-index.md`의 *TODO 섹션*만 수정한다. Slices·Decisions 섹션 본문 및 `tdd-state/slice-N.md`는 Relevant Files에 포함만 하고 읽기 전용.
...
- **Slices · tdd-state/는 읽기만**: handoff는 task-index.md의 TODO 섹션만 수정한다. slice 분해 변경은 `/plan` 재호출, slice 진행 마커는 tdd가 토글, behavior 진행은 tdd-state/slice-N.md의 자동 갱신을 통해서만 일어난다.
