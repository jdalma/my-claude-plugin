---
name: plan
description: 새 기능/이슈를 vertical slice로 분해하고 slice-tdd 구현 준비. 사용자가 "/plan" 슬래시 호출 또는 "기능 슬라이싱", "새 기능 분해", "vertical slice", "task-index.md 만들어줘" 같이 명시 의도를 표현했을 때만 사용. 모호함 해소 → 슬라이싱 → features/<feature-name>/task-index.md publish → slice-tdd 안내까지 수행한다. 자동 호출·자동 제안 금지.
...
## Step 1: 가정 강제 열거 → 라우팅 (단일 표)

> **라우터 질문**: 이 plan이 깔고 들어가는 *모든 가정·결정*에 대해 — **"이걸 판단할 입력이 지금 손에 있나, 외부/미래에서 와야 하나?"**

먼저 이 plan이 전제하는 가정을 **빠짐없이 한 줄씩 나열**한다 (식별자 체계, 멱등키, 동기/비동기, 외부 API 사용 여부, 트래픽 규모, spec 필드 등). "당연하다"고 느끼는 것일수록 명시 대상이다 — *적지 않은 가정은 라우팅도 검증도 못 한다(invisible).* 그 다음 각 행을 표로 강제 채워 **lock / soft / hard** 하나로 라우팅한다:

| 가정/결정 | 종류 | 라우팅 |
|----------|------|--------|
| (예) 외부 API X 사용 여부 | 사실 (조회로 답) | **lock** → 미사용 |
| (예) OpenAPI spec 필드 | 사실이나 외부 종속 | **hard** → spec 팀 합의 안건 |
| (예) 식별자 체계 | 판단 (cross-cutting) | **hard** → grill 후 Decisions 기록 |
| (예) 캐시 TTL | 판단 (기본값 有·슬라이스 한정) | **soft** → 기본값 |

**라우팅 규칙 (겹침 없음):**
- **lock** — *조회로 답 나오는 사실* (코드 grep·사용자 한 마디로 즉시 확정. "GlobalE 부르나?"). → 표에서 바로 확정. 코드 짜서 부딪혀 알아내지 않는다.
- **나머지 전부는 기존 soft/hard** (조회로 안 나오는 *판단*, 또는 외부/미래 종속):
  - **soft**: ① 합리적 기본값(업계 관례·기존 결정) 존재 + ② 영향 해당 슬라이스 한정 (둘 다 만족) → 기본값으로.
  - **hard**: 둘 중 하나라도 불충분 → Step 2 게이트로. (in-hand 판단도 lookup으로 안 나오면 여기로 간다 — grill→Decisions 기록이 *판단의 lock*. 식별자 churn은 이 경로 누락에서 났다.)
...
- [ ] 1. **[tracer bullet]** [한 줄 설명]
       ...
       AFK: yes/no
       depends on: -
       state-file: tdd-state/slice-1.md  (slice-tdd 첫 호출 시 생성)
- [ ] 2. ...
       depends on: 1
...
(plan/slice-tdd 진행 중 누적. 모든 결정은 여기서 관리한다 — 별도 `decisions.md` / `pending-decisions.md` 파일을 만들지 않는다.)
...
출처 태그 (slice 단위까지만):
- `[plan]` — plan 단계에서 결정 (Step 1·2의 hard 모호성 해소 결과)
- `[slice-N]` — 슬라이스 N 작업 중에 결정·발견 (해당 슬라이스 시작/진행/종료 시점 모두 포함)
...
frontmatter의 `feature_name`은 항상 포함한다. takeover의 cross-branch 검색이 이 필드로 동작하므로 누락되면 같은 feature의 이전 handoff와 연결되지 않는다.

`scope-hint`는 plan이 *제안하는 후보*다. 최종 scope 4항목 확정은 **slice-tdd Step 0**에서 수행한다 (plan은 코드를 보지 않으므로 정확한 seam을 알 수 없음).
...
|------|------|-----------|
| `[ ]` | 시작 안 함 | plan 초기 생성 |
| `[~]` | 진행 중 (CURRENT) | slice-tdd 슬라이스 시작 시 (사용자 y/n) |
| `[x]` | 완료 (모든 leaf GREEN + verification 통과) | slice-tdd 슬라이스 완료 시 (사용자 y/n) |
| `[!]` | HARD_BLOCKED 또는 plan 재진입 후보 (예: 깊이 5 초과, 가정 흔들림) | slice-tdd 또는 사용자 |

`features/`는 git 추적 가능한 위치이므로 `.gitignore` 추가 X. 다만 프로젝트 컨벤션에 따라 사용자가 별도 처리 가능.

## Step 5: slice-tdd 핸드오프
...
>
> 다음 명시 호출로 구현 시작:
> - `/slice-tdd` (슬래시 직접 호출) (또는)
> - `slice-tdd로 슬라이스 #1 시작` (또는)
> - `RED→GREEN으로 슬라이스 #1 작업` (또는)
> - `features/<feature-name>/task-index.md 픽업: #1`
>
> 위 키워드 중 하나가 포함되면 **slice-tdd 스킬**이 활성됩니다 (사용자 명시 호출이 있을 때에만).
>
...
다음 모두 충족 시에만 plan을 완료로 본다.

- `features/<feature-name>/task-index.md`가 publish되었거나, 재진입 케이스에서 사용자가 선택한 옵션(overwrite/append/abort/fill) 중 abort 외 하나가 적용됨
- frontmatter에 `feature_name` 필수 필드 포함
- Slices 섹션에 vertical slice 4기준(Vertical / Demoable / Narrow / AFK 우선) 만족하는 항목들이 dependency 순으로 나열됨
- 각 슬라이스에 진행 마커 `[ ]`, `state-file` 경로, scope-hint(`in/out/seam/non-goal` 후보)가 포함됨
- TODO 섹션과 Decisions 섹션 헤더 존재 (내용은 비워둘 수 있음)
- **Step 1의 가정 표가 채워졌고** (모든 가정 행이 lock/soft/hard로 라우팅, lock 행은 확정 값 기재), 그 결과가 `## Open Questions (resolved)` 또는 Decisions 섹션에 반영됨
- hard 모호성이 모두 해소되었거나 `## Open Questions (resolved)` 섹션에 해결 결과 기록됨
- Step 5 slice-tdd 핸드오프 안내 메시지 출력됨
...
**별도 파일 정책**: 모든 결정·트랩은 `task-index.md`의 `## Decisions` 섹션 안에서 관리한다. 별도의 `decisions.md` / `pending-decisions.md` 파일은 사용하지 않는다 (단일 source of truth 원칙). Decisions 섹션 항목은 `[<상태>][<출처>]` 두 태그를 머리에 박는다 — 상태는 `[resolved] / [pending] / [trap]`, 출처는 `[plan] / [slice-N]` (Step 4-3 템플릿 참조).
...
- ❌ `tdd-state/` 디렉토리·파일 생성 (그건 slice-tdd의 역할)
