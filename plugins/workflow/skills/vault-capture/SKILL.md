...
2. **세션 컨텍스트** — 인자가 비어 있으면, 지금까지의 대화에서 raw 재료를 포획한다.
   - **선별 기준**: 아직 파일로 남지 않은, 나중에 `vault-ingest`가 쓸 수 있는 raw
     재료를 고른다 — 결론 안 난 생각, 참고하려고 붙여넣은 자료, 세션에서 나온 아직
     정리 안 된 내용. 정제·요약 없이 있는 그대로 담는다. (ingest처럼 "영속 가치 있는
     결정·근거"로 좁히지 않는다 — capture는 더 느슨하게, 잃어버리지 않는 게 목적이다.)
   - 세션에서 추릴 때는 *무엇을 담을지* 목록을 먼저 보여주고 사용자 확인을 받는다.
     단, 여기서 분류하거나 정제하지 않는다 — "담을지 말지"만 확인한다.
   - 세션 컨텍스트를 **정제·분류해서 vault 본체(Note/Decision)에 넣고 싶다면**
     이 스킬이 아니라 `vault-ingest`를 쓴다. capture는 그 앞단의 "담아두기"만 한다.
...
- 원문성을 보존한다. raw 입력을 capture 단계에서 재작성·요약하지 않는다.
  - **세션 컨텍스트 캡처 시**: 원문 발화를 요약 없이 인용 블록으로 담는다. 전체
    분량이 많으면 사용자가 확인한 항목만 인용한다 — 편집·재서술하지 말고, 담을지
    말지로만 범위를 좁힌다 ("내가 압축해서 정리"가 아니라 "그대로 잘라 담기").
