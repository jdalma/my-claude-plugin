...
| add-worker 도중 팀 shutdown | 페인 생성 후 manifest append 직전 manifest를 **reload** — 사라졌으면("team shut down mid-operation") 새 페인 kill + abort. 락은 없음(레포의 직렬-인간 모델) |

## 7.5 mid-session 워커 추가 (`add-worker`)

실행 중인 팀에 워커 하나를 더 붙이는 경로. `start`가 토폴로지 전체를 한 번에 굽는 것과 달리, `add-worker`는 이미 살아있는 세션에 페인 하나를 split 한다.

- **`createTeamSession`을 재사용하지 않는다.** 그 함수는 `detectTeamMultiplexerContext()`로 호출자 셸의 `env.TMUX`를 읽는데(`tmux-session.js:197`), `add-worker` 프로세스는 팀 tmux 세션 **밖**에서 돈다. 재사용하면 두 번째 고아 세션이 생긴다. 대신 manifest에서 가져온 기존 워커 페인 id를 앵커로 `addWorkerPane(session_name, anchorPaneId, worker)`가 직접 `split-window` 한다.
- **전달(delivery)의 단일 source는 `manifest.workers` append.** `send-message`는 매 호출 sender/recipient의 roster 멤버십을 enforce하므로(`send-message.js:58,65`), 새 워커를 `manifest.workers`에 넣는 것이 새 워커로/에서의 메시징의 **필요충분조건**이다. AGENTS.md는 이 경로에 없다.
- **인지(awareness)는 새 워커 D의 mailbox 인사가 전담.** 이미 떠 있는 기존 워커 LLM은 launch 시 한 번 읽은 AGENTS.md로 peer 목록을 고정한다. 디스크의 AGENTS.md를 다시 써도 런타임 LLM에 도달하지 못하므로 **재작성하지 않는다.** 대신 D가 부팅 직후 받는 유일한 능동 신호인 startup notice가 D에게 "기존 워커 *각각*에게 `send-message`로 인사하고 `expects_reply=true`로 ACK를 받아라"를 지시한다. mailbox 경로라 바쁜 기존 워커도 다음 self-poll 때 **반드시** 받는다(best-effort in-pane notice의 유실 문제 해소). 기존 워커는 자기 AGENTS.md에 이미 있는 `expects_reply` 응답 규율로 답하므로 **기존 워커는 무변경.**
  - **왜 in-pane notice가 아니라 D가 보내나.** 과거엔 add-worker(코드)가 기존 워커 페인에 직접 `New peer available` notice를 tmux로 쳤지만 best-effort라 바쁜 페인에서 유실됐다. 이제 mailbox(신뢰성)로 옮기되, mailbox 메시징은 **워커가** 하는 일이므로(add-worker는 워커가 아니라 외부 명령) D 본인이 보낸다. add-worker가 코드로 D를 흉내내 mailbox에 쓰면 ACK 루프가 껍데기가 된다 — CLI가 곧 종료돼 응답을 기다릴 주체가 없기 때문. D가 살아서 보내야 진짜 ACK 추적이 성립한다.
  - **ACK는 재전송이 아니라 가시성이다.** 답 없는 `sent_pending`은 self-poll 규율상 사용자에게 *표면화*될 뿐 자동 재전송·재시도하지 않는다(my-team은 freeze-wait도 auto-retry도 하지 않는다). 즉 ACK는 "어느 기존 워커가 아직 D를 인지하지 못했나"를 사람이 볼 수 있게 해주는 것이지, 전달 자체를 한 번 더 보장하는 게 아니다. 전달 보장은 mailbox(spool 영속)가 이미 한다.
...
- **권한 플래그(`--launch-arg`).** start의 `launch_args`와 동등하게, add-worker도 `--launch-arg`(반복)로 권한 우회 플래그(예: `--dangerously-skip-permissions`)를 워커 CLI에 forward한다. **이 플래그가 없으면 추가된 워커는 supervised 모드**로 떠서 첫 권한 프롬프트에서 멈춘다 — 사용자가 해당 pane에서 응답해야 진행된다. 자율 워커를 원하면 `--launch-arg`로 우회 플래그를 넘긴다. (start과 달리 `--env`는 의도적으로 제외 — 최소 표면. `--description`은 로스터용, `--extra-prompt`는 Role Context 브리핑용으로 받는다.)
- **`--worktree <branch>` (한 레포 병렬 작업).** `--cwd`의 git 레포에서 `git worktree add <root>/.worktrees/<name> [-b] <branch>`를 실행하고 그 경로를 워커 cwd로 쓴다. `.worktrees/`는 `.git/info/exclude`에 추가해 메인 체크아웃의 `git status`를 더럽히지 않는다. 페인 split 이후 실패 시 방금 만든 워크트리도 `git worktree remove --force`로 되돌린다. 병합 후 워크트리 제거는 사용자 몫이며 `shutdown`은 pane만 죽인다. 워커가 직접 이 명령을 호출할 수 있도록 AGENTS.md에 레시피가 들어가며(허용된 유일한 비-api 서브커맨드), `--launch-arg` 없이 호출하면 호출한 워커(`MY_TEAM_WORKER`)의 `launch_args`(manifest에 저장)를 상속한다. 같은 레포 워커끼리 충돌을 협의해야 하므로 팀 내부 worker→worker 발신 제한은 제거됐다 — role 가드는 cross-team에만 적용된다.
